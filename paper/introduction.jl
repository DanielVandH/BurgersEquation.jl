...
for k in 1:length(μ)
    ...
    lines!(ax, x, u_inviscid, color=:red, linestyle=:dash, linewidth=1.3)
    for j in 1:length(t)
        lines!(ax, x, u_viscous[:, j, k], color=:black, linestyle=:solid, linewidth=1.3)
    end
    lines!(ax, x, u_large[:, k], color=:blue, linestyle=:dash, linewidth=1.3)
end
